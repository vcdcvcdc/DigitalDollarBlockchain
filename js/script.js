...
// let props = { d: d, m: m, s: s };

// function setup() {
//   let canvas = createCanvas(window.innerWidth, window.innerHeight);

//   // Randomize params on click
//   // canvas.mousePressed(() => {
//   //   props.m = round(random(3, 8));
//   //   props.d = random(2, 4);
//   //   props.s = random(1, 2.5);
...
//   // gui.domElement.addEventListener("click", () => {hue = random(1); initialize()});
// }
...
//     const c = round(random(2 * m));
//     const x1 = x / (x * x + y * y) / d + a[c];
//     const y1 = y / (x * x + y * y) / d + b[c];
//     x = x1 / (x1 * x1 + y1 * y1);
//     y = y1 / (x1 * x1 + y1 * y1);
//     point(x * zoom, y * zoom);
//   }
...
//   background(0);
//   translate(width / 2, height / 2);
//   stroke(hue, 0.8, 1, 0.1);

//   a = new Float32Array(m * 2 + 1);
//   b = new Float32Array(m * 2 + 1);

//   for (let i = 0; i < m; i++) {
//     a[2 * i] = cos((TWO_PI * (i + 1)) / m);
//     b[2 * i] = sin((TWO_PI * (i + 1)) / m);

//     a[2 * i + 1] = 0.5 * (cos((TWO_PI * (i + s)) / m) + cos((TWO_PI * i) / m));
//     b[2 * i + 1] = 0.5 * (sin((TWO_PI * (i + s)) / m) + sin((TWO_PI * i) / m));
//   }
// }
